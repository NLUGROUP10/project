...
class SinusoidPositionalEncodings(nn.Module):
...
class TreeAttention(nn.Module):
    def __init__(self, config):
        ...
        self.d_model = config.decoder_d_model
        self.key_value_proj_dim = config.decoder_d_kv
        self.n_heads = config.decoder_num_heads
        self.dropout = config.decoer_dropout_rate
        self.inner_dim = self.n_heads * self.key_value_proj_dim
        self.rel_vocab_size = config.rel_vocab_size
    ...
    def forward(
        ...
        if rel is not None:
            scores = scores + rel * mask if self.additive else scores * (rel * mask)
        else:
            scores = scores + mask
...
class TreeLayerSelfAttention(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__()
        self.SelfAttention = TreeAttention(config)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class TreeLayerCrossAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.EncDecAttention = TreeAttention(config)
        self.layer_norm = T5LayerNorm(config.d_model, eps=config.layer_norm_epsilon)
        self.dropout = nn.Dropout(config.dropout_rate)
...
class TreeDecoderBlock(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__()
        self.is_decoder = config.is_decoder
        self.layer = nn.ModuleList()
        self.layer.append(TreeLayerSelfAttention(config))
        if self.is_decoder:
            self.layer.append(TreeLayerCrossAttention(config))
...
class TreeDecoderStack(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.n_embd = config.n_embd
        self.is_decoder = config.is_decoder
        self.types_vocab_size = config.types_vocab_size
        self.values_vocab_size = config.values_vocab_size
        ...
        self.wte_types = nn.Embedding(self.types_vocab_size, self.n_embd)
        self.wte_values = nn.Embedding(self.values_vocab_size, self.n_embd)
        self.tree_positional_encodings = TreePositionalEncodings(config.n_embd, config.width,config.depth)
        self.encoder_sin_positional_encodings = SinusoidPositionalEncodings(config.d_model,n_position=1000)


        self.block = nn.ModuleList(
            [TreeDecoderBlock(config) for i in range(config.num_layers)]
        )
    ...
    def forward(
        ...
        tree_emd = self.tree_positional_encodings(tree_code)
        inputs_embeds = types_embeds + values_embeds+tree_emd

        if encoder_hidden_states is not None:
            encoder_hidden_states = self.encoder_sin_positional_encodings(encoder_hidden_states)
